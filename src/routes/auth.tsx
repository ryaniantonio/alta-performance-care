...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Acessar painel — Ryani Nutricionista" },
      ...
      <aside className="relative hidden lg:flex flex-col justify-between p-12 bg-gradient-to-br from-emerald-50 via-white to-emerald-100/40 dark:from-emerald-950/40 dark:via-background dark:to-emerald-900/20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-md space-y-6">
          ...
          <p className="text-muted-foreground">
            Agenda, prontuários e planos alimentares em um único painel — para que você foque no que
            importa: o cuidado.
          </p>
          <div className="flex items-center gap-3 text-sm">
            <div className="flex -space-x-2">
              {["E", "M", "L"].map((c) => (
                <div
                  key={c}
                  className="h-8 w-8 rounded-full bg-foreground text-background grid place-items-center text-xs font-medium ring-2 ring-background"
                >
                  {c}
                </div>
              ))}
        ...
        <p className="relative z-10 text-xs text-muted-foreground">
          © {new Date().getFullYear()} Ryani Nutricionista
        </p>
      </aside>
